...
void Timer0_Handler(nrf_timer_event_t et,void *p_context)
{
    ...
    #if (defined(DEF_OSAL_EN)&&(DEF_OSAL_EN==1))
    {
        if(timerActive==TRUE)
        {
            if(Usr_Osal_Timestamp_Real>=DEF_OSAL_TICK_MS)
            {
                Usr_Osal_Timestamp_Real = 0;
                osal_update_timers();
            }
        }
        else
...
void Usr_Timer_InitSetup(void)
{
    ...
    // err_code = nrf_drv_timer_init(&Timer0_St, NULL, Timer0_Handler_Ptr);
